...
function productlist() {
    ...
    const FetchBestSellers = async () => {
        ...
        try {
            const response = await ManageAPIsData(`${FRONTEND_CAMPAIGN_ENDPOINT}?id=${id}`, 'GET');
            if (!response.ok) {
                console.error('Error fetching data:', response.statusText);
                return;
            }
            ...
            if (responseData.data.products.length) {
                setcampaignDetails(responseData.data)
    ...
    const AddToCart = async (productDetails) => {
        try {
            if (session?.user?.id) {
                ...
                enqueueSnackbar('Product added to cart (local storage)!');
            }
        } catch (error) {
                            ...
                            <div id="productListBanner">
                                <div className="row">
                                    <div className="col-md-12">
                                        <img src={`${governmentBasePath}/${campaignDetails?.banner_img}`} alt="" height="500" />
                                    </div>
                                </div>
                            </div>
                            {/* filter product ends */}
                                                                ...
                                                                <div className="left-slide">
                                                                    {selectedProduct.images.map((image, index) => (
                                                                        <div key={index} className={`mySlides ${index === 0 ? 'active' : ''}`}>
                                                                            <img src={`${governmentBasePath}/${image.url}`} alt="" />
                                                                        </div>
                                                                    ))}
                                                                </div>
                                                                        ...
                                                                        {selectedProduct.images.map((image, index) => (
                                                                            <div key={index} className="column">
                                                                                <img
                                                                                    ...
                                                                                    src={`${governmentBasePath}/${image.url}`}
                                                                                    onClick={() => { }}
                                                                                    alt=""
                                                                                />
                                                                            </div>
                                            ...
                                            <div>
                                                <Card style={{ backgroundColor: 'white', color: "black" }}>
                                                    <CardContent>
                                                        ...
                                                        <p><strong>Start Date:</strong> {fDate(campaignDetails?.start_date)}</p>
                                                        <p><strong>End Date:</strong> {fDate(campaignDetails?.till_date)}</p>
                                                        <p><strong>Discount Percentage:</strong> {campaignDetails?.discount_percentage}%</p>
                                                        <p><strong>Coupon Code:</strong> {campaignDetails?.coupon_code}</p>
                                                    ...
                                                    <div className="col-lg-3 col-md-6 col-sm-12 col-xs-12 product-container mt-4">
                                                        ...
                                                        <div className="product-img-section">
                                                            <Link href={`/productdetails/${prdct.id}`}>
                                                                <img src={`${governmentBasePath}/${prdct?.images[0]?.url}`} alt="" className="product-img" height="320" />
                                                            </Link>
